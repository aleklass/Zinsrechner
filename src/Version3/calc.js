...
(function (root, factory) {
  ...
  function simulateNextForrest(v) {
    ...
    let teamQualified = false;
    let teamQualifiedFromMonth = null;
    let teamTotalTippgeberBonus = 0;
    let teamTotalOverrideBonus = 0;
    const teamRows = [];
    ...
    for (let m = 1; m <= months; m++) {
      ...
      if (hasTeam) {
        ...
        teamSimulated.forEach((sim) => {
          teamVolume += activeAt(sim, m);
          if (Math.round(num(sim.member.level) || 1) !== 1) return;

          level1Active += activeAt(sim, m);
        ...
        if (
          !teamQualified &&
          activeCapital >= TEAM_OWN_CAPITAL_THRESHOLD &&
          teamVolume >= TEAM_VOLUME_THRESHOLD
        ) {
          teamQualified = true;
          teamQualifiedFromMonth = m;
        }

        const overrideBonus = teamQualified
          ? level1Active * TEAM_LEVEL1_OVERRIDE_PCT
          : 0;
        const totalBonus = tippgeberBonus + overrideBonus;
        teamTotalTippgeberBonus += tippgeberBonus;
        teamTotalOverrideBonus += overrideBonus;
        cash += totalBonus;
    ...
    return {
      ...
      team: hasTeam
        ? {
            rows: teamRows,
            qualifiedFromMonth: teamQualifiedFromMonth,
            totalTippgeberBonus: teamTotalTippgeberBonus,
            totalOverrideBonus: teamTotalOverrideBonus,
            totalBonus:
              teamTotalTippgeberBonus + teamTotalOverrideBonus,
          }
        : null,
    };
  }
  ...
  // Ebene-1-Empfehlung investiert, und — sobald sowohl das eigene aktive
  // Kapital als auch das Team-Volumen über alle Ebenen die Schwelle
  // erreichen — eine laufende monatliche Beteiligung auf das aktive Kapital
  // der Ebene-1-Empfehlungen.
  const TEAM_BLOCK_BONUS = 50;
  const TEAM_LEVEL1_OVERRIDE_PCT = 0.01;
  const TEAM_OWN_CAPITAL_THRESHOLD = 10000;
  const TEAM_VOLUME_THRESHOLD = 100000;
  ...
  return {
    ...
    NF_BLOCK,
    TEAM_BLOCK_BONUS,
    TEAM_LEVEL1_OVERRIDE_PCT,
    TEAM_OWN_CAPITAL_THRESHOLD,
